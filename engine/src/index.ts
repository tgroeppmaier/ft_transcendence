...
setInterval(() => {
  ...
  if (ball.x - radius <= -1 + leftPaddle.w && ball.y >= leftPaddle.y && ball.y <= leftPaddle.y + leftPaddle.h) {
    ball.x = -1 + leftPaddle.w + radius;  // Position ball just outside the paddle
    ball.vx *= -1;
